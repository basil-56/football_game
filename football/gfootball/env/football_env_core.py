...
####################################################################################################################
  ...
  def __del__(self):
    self.close()
#imp not calling named_action_from_action_set(for log action)
  def step(self, action, extra_data={}):
    ...
    if episode_done:
      del self._trace
      self._trace = None
      fps = 10#self._step_count / (debug['time'] - self._episode_start)
      #print('fps',fps)
      game_fps = 10#self._step_count / self._steps_time
      logging.info(
